const CACHE = 'camutanga-v43-correcao-total';
const SHELL = [
...
self.addEventListener('install', event => {
  event.waitUntil((async()=>{
    ...
    await Promise.all(SHELL.map(async url => {
      try {
        const r = await fetch(url + (url.includes('?') ? '&' : '?') + 'v=43', {cache:'no-store'});
