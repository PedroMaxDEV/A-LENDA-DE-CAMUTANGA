const CACHE = 'camutanga-v44-passagens-autosave';
const SHELL = [
...
self.addEventListener('install', event => {
  event.waitUntil((async()=>{
    ...
    await Promise.all(SHELL.map(async url => {
      try {
        const r = await fetch(url + (url.includes('?') ? '&' : '?') + 'v=44', {cache:'no-store'});
